...
fn test_preview_search_overlay_highlight() {
    use ascope::app::{ModalMode, SearchOverlayMode, SearchMatch};
    use std::fs::File;
    use std::io::Write;
    ...
    state.search_overlay_results = vec![
        SearchMatch {
    ...
    let preview_text: String = lines.iter().map(|l| l.to_string()).collect::<Vec<_>>().join("\n");
    assert!(preview_text.contains("line 3"), "Preview should contain the matched line content");
    ...
    let new_preview_text: String = new_preview.iter().map(|l| l.to_string()).collect::<Vec<_>>().join("\n");
    assert!(new_preview_text.contains("line 5"), "Preview should update to center on the new line");
}

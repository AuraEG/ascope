...
fn test_plugin_engine_search_api() {
    ...
    fs::copy(plugin_dir.join("plugin.toml"), dest_plugin.join("plugin.toml")).unwrap();
    fs::copy(plugin_dir.join("init.lua"), dest_plugin.join("init.lua")).unwrap();

    let mut engine = PluginEngine::new(config_dir).unwrap();
    engine.load_plugins().unwrap();

    let res = engine.trigger_event("test_search", "special query".to_string()).unwrap();
    assert_eq!(res, vec!["found: this is a special query string match\n".to_string()]);
}

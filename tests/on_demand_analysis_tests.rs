use ascope::{app::AppState, app::ModalMode, fs::walker::EntryType};
...
fn test_lazy_loading_and_popup_transitions() {
    ...
    loop {
        state.poll_scan();
        if state.all_entries.iter().any(|e| e.path.file_name().unwrap() == "subdir1") {
            break;
        }
    ...
    assert!(!state.all_entries.iter().any(|e| e.path.file_name().unwrap() == "file1.txt"),
            "file1.txt should not be loaded on startup (lazy loading)");

    // Select the directory and expand it
    let subdir_idx = state.all_entries.iter()

                    ...
                    if state.modal_mode == ascope::app::ModalMode::SearchOverlay {
                        if state.search_overlay_focused {
                            match key.code {
                                ...
                                KeyCode::Tab => {
                                    if state.search_overlay_mode == ascope::app::SearchOverlayMode::FuzzyFiles {
                                ...
                                KeyCode::Right => {
                                    if state.search_overlay_cursor_index < state.search_overlay_input.chars().count() {
                                        state.search_overlay_cursor_index += 1;
                                    }
                                }
                                KeyCode::Enter => {
                                    if let Some(target) = state.search_overlay_results.get(state.search_overlay_selected_index).cloned() {
                                        state.modal_mode = ascope::app::ModalMode::None;
                                        state.search_overlay_input.clear();
                                        state.search_overlay_results.clear();
                                        state.search_overlay_cursor_index = 0;
                                ...
                                KeyCode::Char(c) => {
                                    ...
                                    {
                                        let char_idx = state.search_overlay_cursor_index;
                                        let byte_idx = state.search_overlay_input.char_indices().nth(char_idx).map(|(i, _)| i).unwrap_or(state.search_overlay_input.len());
